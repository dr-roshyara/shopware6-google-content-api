...
class DefaultTranslationProvider
{
     ...
     *
     * See also: https://github.com/shopware/platform/blob/0aaa7c6ba42ac31a16176a9ae5c2c201b13aacbc/src/Core/Framework/DataAbstractionLayer/FieldSerializer/TranslationsAssociationFieldSerializer.php#L213-L218
    ...
    private function ensureSystemDefaultTranslationInSingleEntityWritePayload(
        ...
        foreach ($associationFields as $associationField) {
            ...
            if (array_key_exists($associationField->getPropertyName(), $payload)) {
                ...
                } elseif ($isToManyAssociationField) {
                    foreach ($payload[$associationField->getPropertyName()] as $key => &$singleAssociationPayload) {
                        $this->ensureSystemDefaultTranslationInSingleEntityWritePayload(
                            $associatedClass,
