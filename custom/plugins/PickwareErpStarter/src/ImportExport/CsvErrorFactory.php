...
class CsvErrorFactory
{
    ...
    public static function invalidCellValue(ValidationError $error): JsonApiError
    {
        $jsonError = [
            'code' => self::ERROR_CODE_UNKNOWN_ERROR,
            ...
            'meta' => [
                'validationType' => $error->keyword(),
                'normalizedColumnName' => $error->dataPointer()[0],
            ],
        ];
        ...
        switch ($error->keyword()) {
            case 'type':
                ...
                $jsonError['meta']['expected'] = $error->keywordArgs()['expected'];
                $jsonError['meta']['actual'] = $error->keywordArgs()['used'];
                break;
            case 'minimum':
                $jsonError['code'] = self::ERROR_CODE_ROW_VALIDATION_FAILED_LIMIT_ACTUAL;
                $jsonError['meta']['limit'] = $error->keywordArgs()['min'];
                $jsonError['meta']['actual'] = $error->data();
                break;
            case 'minLength':
                $jsonError['code'] = self::ERROR_CODE_ROW_VALIDATION_FAILED_LIMIT_ACTUAL;
                $jsonError['meta']['limit'] = $error->keywordArgs()['min'];
                $jsonError['meta']['actual'] = $error->keywordArgs()['length'];
                break;
            case 'maxLength':
                $jsonError['code'] = self::ERROR_CODE_ROW_VALIDATION_FAILED_LIMIT_ACTUAL;
                $jsonError['meta']['limit'] = $error->keywordArgs()['max'];
                $jsonError['meta']['actual'] = $error->keywordArgs()['length'];
                break;
            case 'oneOf':
