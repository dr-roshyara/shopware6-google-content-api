...
use Pickware\PickwareErpStarter\ImportExport\Profiles\SupplierOrder\SupplierOrderExporter;
...
class Migration1645103528UpdateSupplierOrderPriceFields extends MigrationStep
{
        ...
        if (count($supplierOrders) > 0) {
            ...
            foreach ($supplierOrders as $supplierOrder) {
                ...
                $sqlUpdateValues[] = [
                    ...
                    sprintf('\'%s\'', json_encode($cartPrice, \JSON_UNESCAPED_UNICODE | \JSON_PRESERVE_ZERO_FRACTION | \JSON_THROW_ON_ERROR | \JSON_INVALID_UTF8_IGNORE)),
    ...
    private function migrateSupplierOrderLineItemPrices(Connection $connection): void
    {
        ...
        if (count($supplierOrderLineItems) > 0) {
            ...
            foreach ($supplierOrderLineItems as $supplierOrderLineItem) {
                ...
                $sqlUpdateValues[] = [
                    ...
                    sprintf('\'%s\'', json_encode($calculatedPrice, \JSON_UNESCAPED_UNICODE | \JSON_PRESERVE_ZERO_FRACTION | \JSON_THROW_ON_ERROR | \JSON_INVALID_UTF8_IGNORE)),
                    sprintf('\'%s\'', json_encode($priceDefinition, \JSON_UNESCAPED_UNICODE | \JSON_PRESERVE_ZERO_FRACTION | \JSON_THROW_ON_ERROR | \JSON_INVALID_UTF8_IGNORE)),
        ...
        $connection->executeStatement(
            'ALTER TABLE `pickware_erp_supplier_order_line_item`
            ADD COLUMN `quantity` INT GENERATED ALWAYS AS (JSON_UNQUOTE(JSON_EXTRACT(`price`,"$.quantity"))) VIRTUAL AFTER `price`;',
        );
    }
    ...
    private function migrateExportColumnConfiguration(Connection $connection): void
    {
        ...
        $configValue = json_decode($existingConfiguration[0]['configValue'], true);
        ...
        $connection->executeStatement(
            ...
            [
                'newConfigurationValue' => json_encode([ConfigJsonField::STORAGE_KEY => $columns]),
                'configurationKey' => SupplierOrderExporter::PLUGIN_CONFIG_COLUMNS_KEY,
            ],
        );
    }
}
