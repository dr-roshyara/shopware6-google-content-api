...
use Pickware\PickwareErpStarter\InvoiceCorrection\InvoiceCorrectionDocumentConfigurationValidatingDocumentServiceDecorator;
...
class InvoiceStackService
{
    ...
    public function getInvoiceStacksOfOrder(string $orderId, Context $context): InvoiceStackCollection
    {
        ...
        foreach ($documents->getElements() as $documentEntity) {
            if ($documentEntity->getDocumentType()->getTechnicalName() === InvoiceGenerator::INVOICE) {
        ...
        foreach ($nonInvoiceDocumentEntities as $nonInvoiceDocumentEntity) {
            ...
            if ($type === InvoiceCorrectionDocumentType::TECHNICAL_NAME) {
                $invoiceNumber = $nonInvoiceDocumentEntity->getConfig()['custom'][InvoiceCorrectionDocumentConfigurationValidatingDocumentServiceDecorator::DOCUMENT_CONFIGURATION_REFERENCED_INVOICE_NUMBER_KEY];
            ...
            if ($type === StornoGenerator::STORNO) {
